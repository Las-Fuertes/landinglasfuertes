...
1. **Elegir nivel de reposo.** `?reposo=medio` (por defecto) y `?reposo=alto` siguen los dos
   disponibles para comparar (D8, regla 12: temporal). Falta que Johan elija uno y se borre el
   parámetro y el otro nivel.
2. Pendiente anotado, no pedido: en pantallas más bajas que el lienzo (portátil de 1280x700, móvil
   apaisado) la parte se centra y se recorta. Ver el final de D2. Lo decide Johan.
3. Pendiente anotado, no pedido: cambio de fuente de acento de Homemade Apple a Bradley Hand en
