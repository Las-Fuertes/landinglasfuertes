...
0. **Las transiciones de la Introducción.** Pedido por Johan el 2026-09-22 y marcado por él como
   muy importante. **Las 3 partes ya están aprobadas y no se rediseñan**; lo que falta es cómo se
   pasa de una a la siguiente: la intro se queda fija en pantalla y el scroll no desplaza el
   contenido, cambia de parte, con una salida (las piezas se van, el texto se desvanece) y una
   entrada por cada cambio; reversible hacia arriba. **Se arranca en un chat nuevo**, con el
   prompt de `CONTINUAR.md`. Aviso que va en ese prompt: la intro vieja con GSAP + ScrollTrigger
   del commit `92fda5e` es de un diseño anterior que ya no existe; no se restaura ni se usa de
   base.
