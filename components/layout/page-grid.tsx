 ...
 * On very small phones (<380px) the inset drops to 24px to give text more room.
 */
export function PageGrid({ children, className = '', ...rest }: PageGridProps) {
  return (
    <div
      className={`mx-auto grid w-full max-w-[1200px] grid-cols-4 gap-x-grid-gutter gap-y-grid-gutter px-6 min-[380px]:px-page-margin md:grid-cols-12 ${className}`.trim()}
      {...rest}
    >
      {children}
    </div>
